import { criarCena, type Ponto3D, type PontoTela } from './iso';
import type { AmbienteId, SistemaId } from './cenarios';

export interface CenaGerada {
  svg: string;
  pontos: PontoTela[];
  /** Deslocamento do rótulo em relação ao ponto [dx, dy, índice do ponto daquele sistema]. */
  rotulos: Record<SistemaId, [number, number, number]>;
}
...
function condominio(): CenaGerada {
  ...
  const hub: Ponto3D = [15.6, 14.2, 2.85];
...
export const cenas: Record<AmbienteId, () => CenaGerada> = { casa, comercio, condominio, empresa };
