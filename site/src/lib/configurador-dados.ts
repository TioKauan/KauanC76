...
export type AmbienteConfigId = 'casa' | 'comercio' | 'empresa' | 'condominio';
export type TipoPlanta = 'casa' | 'comercial';
/** Texto fixo ou variando por ambiente (comércio × empresa usam a mesma planta). */
export type TextoPorAmbiente = string | Partial<Record<AmbienteConfigId, string>>;
export interface Retangulo { x: number; y: number; w: number; h: number }
export interface Zona extends Retangulo { nome: TextoPorAmbiente }
export interface Sugestao { id: string; nome: TextoPorAmbiente; x: number; y: number; angulo: number }
export interface Planta {
  /** Posição do gravador (DVR). */
  dvr: [number, number];
  /** Conversão de unidades do desenho para metros de cabo. */
  metrosPorUnidade: number;
  /** Metros extras por câmera (subida, sobra de instalação). */
  folga: number;
  edificios: Retangulo[];
  zonas: Zona[];
  sugestoes: Sugestao[];
...
export interface AmbienteConfig { id: AmbienteConfigId; nome: string; icone: NomeIcone; planta: TipoPlanta | null }
export interface SistemaCondominio { id: string; nome: string; icone: NomeIcone }

export const ambientesConfig: AmbienteConfig[] = [
...
export const LOTE: Retangulo = { x: 24, y: 22, w: 770, h: 440 };
export const MAX_CAMERAS = 16;

export const plantas: Record<TipoPlanta, Planta> = {
  casa: {
    dvr: [340, 250],
...
export const sistemasCondominio: SistemaCondominio[] = [
...
export const nomeEm = (valor: TextoPorAmbiente, ambiente: AmbienteConfigId): string =>
  typeof valor === 'string' ? valor : valor[ambiente] ?? Object.values(valor)[0] ?? '';
