...
Qualquer uma pode ser revista: a maioria muda em um único arquivo (`site/src/dados.js`).
...
**Como mudar:** em `site/src/dados.js`, `mostrarPrecos: false` troca todos os valores por "Sob consulta".
...
**Como mudar:** o número fica em `dados.js` (`contato.whatsapp`).
...
| Tema | Decisão | Motivo |
|---|---|---|
| Tecnologia | Vite + JavaScript e CSS puros, sem framework | Mais simples de manter; gera HTML estático para o Nginx |
| Animações | CSS + Web Animations API, sem GSAP | Mais leve; tudo desliga com "reduzir movimento" |
| Linha do tempo | JavaScript (IntersectionObserver) em vez de `animation-timeline` | A versão só com CSS ainda não roda em todos os navegadores (~84%) |
| Pasta `site/` | Separada do projeto atual | Evita conflito se o projeto React/Vinext vier para este repositório |
